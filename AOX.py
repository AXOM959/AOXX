...
#AC00FF
      ||||||||||||||||||||||||||||||||||||||||||||
    |||||||||||||Technology_By_Draken_Kun|||||||||||||
  ||||||||||GITHUB_:https://github.com/AXOM959||||||||||
||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
       ||||||||||||||||||||||||||||||||||||||||||||
...
def crack(idf,pwv):
    ...
    sys.stdout.write(f"\r [SEARCH] {P}{k}\033[1;37m{loop}\033[1;31m{P}  {P}{H}[ok] - {ok}{P}]-{P}[{k}[cp] {cp}{x}]-[{bo}{'{:.0%}'.format(loop/float(len(id)))}{P}]     ")
    sys.stdout.flush()
